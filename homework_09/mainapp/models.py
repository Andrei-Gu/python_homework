...
class InstrumentalResearchCategory(models.Model):
    ...
    def get_absolute_url(self):
        return "/ir-category/%i/" % self.id


    def get_short_name_url(self):
        return "/ir-category/%s/" % self.short_name
...
class InstrumentalResearch(models.Model):
    ...
    def get_absolute_url(self):
        return "/ir-category/%s/" % f'{self.i_r_category.short_name}/{self.id}'
...
class InstrumentalResearchCard(models.Model):
    ...
    i_r_interesting_fact = models.TextField(max_length=1000, blank=True)
